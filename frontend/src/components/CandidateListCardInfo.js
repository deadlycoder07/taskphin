...
const CandidatesListCardInfo = ({candidateData}) => {
    ...
    const handleChange = async (e) => {
      ...
      try {
        const response = await axios.patch('http://localhost:3000/api/v1/candidates', {'id':candidateData.id,candidateStatus:e.target.value});
        // Handle success
    ...
    <div className="w-full h-40 bg-white shadow-md rounded-lg overflow-hidden m-4 p-4 flex items-center">
      ...
      <div className="flex-grow flex flex-col justify-between m-6">
        {/* Apply space-y-* utility here for vertical spacing */}
        <div className="flex gap-x-4">
          <div className="flex-1 space-y-2 p-8 text-gray-500"> {/* Add vertical spacing between fields */}
